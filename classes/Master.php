...
Class Master{
	...
	function save_or_payment() {
		...
		if (empty($id)) {
			...
			if ($save) {
				...
				WHERE a.c_or_no = b.c_or_no
				AND a.c_or_no = '$c_or_no' AND a.ctid < b.ctid;";
				odbc_exec($this->conn, $delete_duplicate);
	...
	function save_car_payment() {
		...
		if (empty($c_check_no) && empty($c_ref_no)) {
			$c_check_no = $c_voucher_no;
		} elseif (empty($c_check_no)) {
			$c_check_no = $c_ref_no;
		}
		...
		if (empty($id)) {
			...
			if ($save) {
				...
				WHERE a.c_car_no = b.c_car_no
				AND a.c_car_no = '$c_car_no' AND a.ctid < b.ctid;";
				odbc_exec($this->conn, $delete_duplicate);
					...
					if ($check_items_result) {
						...
						if ($current_car_no_result) {
							...
							if (!in_array($c_car_no, $car_no_array)) {
								...
								$status_value = $count_items > 0 ? 2 : 1;
								$update_atap = "UPDATE t_atap SET status = $status_value, c_car_no = '$new_car_no' WHERE c_atap_no = '$c_atap_no'";
								$update = odbc_exec($this->conn, $update_atap);
							} else {
								$update = true;
							}
			...
			if ($save) {
				...
				// Update transaction type in atap items if applicable
				if (!empty($atap_id)) {
					$update_tran_type = "UPDATE t_atap_items SET atap_status = 1 WHERE id = '$atap_id'";
					$update_tran_type_result = odbc_exec($this->conn, $update_tran_type);
					if (!$update_tran_type_result) {
			...
			if (!empty($selectedRows)) {
				...
				foreach ($selectedRows as $row) {
					...
					$insert_transfer_to_car = "INSERT INTO t_car_payment
						(id, c_account_no, c_car_type, c_car_no, c_car_paydate, c_car_amount, c_encoded_by, c_tran_date, c_tran_updated, c_mop, status, e_status, c_bank, 
						c_check_no, c_remarks, c_atap_no) 
						VALUES ('$cid','$new_acc', '$c_car_type', '$c_car_no', '$c_car_paydate', '$c_car_amount', '$transferred_by', 
