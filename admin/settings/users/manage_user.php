                ...
                <form id="addUserForm" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control" id="c_department" name="c_department" required>
                            ...
                            <?php
                            $department_query = "SELECT DISTINCT c_department, id FROM t_emp_department WHERE status = 0 ORDER BY id ASC";
                            $department_result = odbc_exec($conn, $department_query);
                            while ($dept_row = odbc_fetch_array($department_result)) {
                                $selected = (isset($row['c_department']) && $row['c_department'] == $dept_row['c_department']) ? 'selected' : '';
                                echo "<option value='".htmlspecialchars($dept_row['c_department'], ENT_QUOTES, 'UTF-8')."' $selected>".htmlspecialchars($dept_row['c_department'], ENT_QUOTES, 'UTF-8')."</option>";
                            }
                            ?>
                        </select>
                    </div>
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="dropdown">
                                    <select class="form-control" id="c_position" name="c_position" required>
                                        ...
                                        <?php
                                        $check_type_query = "SELECT DISTINCT c_position, id FROM t_emp_position WHERE status = 0 ORDER BY id ASC";
                                        $type_result = odbc_exec($conn, $check_type_query);
                                        while ($row = odbc_fetch_array($type_result)) {
                                            $selected = (isset($c_position) && $c_position == $row['c_position']) ? 'selected' : '';
                                            echo "<option value='".htmlspecialchars($row['c_position'], ENT_QUOTES, 'UTF-8')."' $selected>".htmlspecialchars($row['c_position'], ENT_QUOTES, 'UTF-8')."</option>";
                                        }
                                        ?>
                                    </select>
                                </div>
                ...
                <form id="editUserForm" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control" id="edit_department" name="c_department" required>
                            ...
                            $department_result = odbc_exec($conn, $department_query);
                            while ($dept_row = odbc_fetch_array($department_result)) {
                                $selected = (isset($c_department) && $c_department == $dept_row['c_department']) ? 'selected' : '';
                                echo "<option value='".htmlspecialchars($dept_row['c_department'], ENT_QUOTES, 'UTF-8')."' $selected>".htmlspecialchars($dept_row['c_department'], ENT_QUOTES, 'UTF-8')."</option>";
                            }
                            ?>
                        </select>
