...
if (isset($_GET['account_no']) && isset($_GET['car_type'])) {
    $c_account_no = $_GET['account_no'];
    $c_car_type = $_GET['car_type'];

    function fetchCarPayments($conn, $c_account_no, $c_car_type) {
        ...
        if (!$stmt || !odbc_execute($stmt, array($c_account_no, $c_car_type))) {
            return null;
        }
    ...
    $carData = fetchCarPayments($conn, $c_account_no, $c_car_type);
