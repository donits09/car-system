...
if ($c_vat_selected === '' || $c_vat_selected === null) {
    $c_vat_selected = '1';
}
            ...
            success: function(response) {
                if (response.status === 'success') {
                    if (response.data && response.data.c_account_no_or) {
                        ...
                        if (response.data.c_account_no_or !== currentAccountNo) {
                            $('#or_type_container').show();
                            $('#tran_type_container_or').hide();
...
function updateORList() {
    ...
    fetch(`<?php echo base_url; ?>supervisor/other_fees/fetch_or_list.php?username=${username}&buyer_acc_no=${accountNo}`)
        .then(response => {
            if (!response.ok) {
